...
// ---------------------------------------------------------------------------
function ReferenceNode({ node, onChange, onMouseDownHeader, onClose, selected, incomingOutputItems }) {
  const accent = "#F59E0B";
  const d = node.data;
  const images = d.images || [];
  const [copied, setCopied] = React.useState(false);
  // Set de URLs seleccionadas del OutputNode conectado
  const [selectedFromOutput, setSelectedFromOutput] = React.useState(() => new Set(d.images || []));

  const hasIncomingOutput = incomingOutputItems && incomingOutputItems.length > 0;

  // Analiza las imágenes pasadas directamente (evita leer state stale)
  const _lastAnalyzedKey = React.useRef(null);
  const analyzeImages = React.useCallback(async (imagesToAnalyze) => {
    if (!imagesToAnalyze.length) return;
    const key = imagesToAnalyze.join("||");
    if (_lastAnalyzedKey.current === key) return;
    _lastAnalyzedKey.current = key;
    ...
    try {
      ...
      const manifest = await window.__auditWithPolling({
        ...
        images: imagesToAnalyze.map((url, i) => ({ id: `r${i}`, url })),
      });
      ...
      window.__notify?.({
        kind: "success", icon: "🎭",
        title: "Personaje analizado",
        body: chars.length ? `${chars.length} personaje(s) detectado(s)` : "Estilo extraído",
      });
    } catch (e) {
      _lastAnalyzedKey.current = null;
      onChange({ status: "idle" });
      window.__notify?.({ kind: "error", icon: "✕", title: "Error al analizar", body: e.message?.slice(0, 80) });
    }
  }, [node.id, d.label, onChange]);

  // Cuando cambian los items del OutputNode, limpiar selecciones que ya no existen
  React.useEffect(() => {
    if (!hasIncomingOutput) return;
    const available = new Set((incomingOutputItems || []).map((it) => it.url));
    setSelectedFromOutput((prev) => new Set([...prev].filter((u) => available.has(u))));
  }, [JSON.stringify((incomingOutputItems || []).map((it) => it.url)), hasIncomingOutput]);

  // Cuando cambia la selección del OutputNode → actualizar images + auto-analizar
  const _autoTimer = React.useRef(null);
  React.useEffect(() => {
    if (!hasIncomingOutput) return;
    const urls = [...selectedFromOutput];
    if (JSON.stringify(urls) === JSON.stringify(d.images || [])) return;
    onChange({ images: urls });
    clearTimeout(_autoTimer.current);
    if (urls.length > 0) {
      _autoTimer.current = setTimeout(() => analyzeImages(urls), 600);
    }
    return () => clearTimeout(_autoTimer.current);
  }, [selectedFromOutput, hasIncomingOutput]);

  const toggleOutput = (url) => {
    setSelectedFromOutput((prev) => {
  ...
  return (
    <div className={"node-v2 reference-node " + (selected ? "is-selected" : "")} data-accent="amber">
      ...
      <div className="node-v2-header" onMouseDown={onMouseDownHeader}>
        ...
        <div className="node-v2-right">
          <span className="mono" style={{ fontSize:9, letterSpacing:"0.14em", textTransform:"uppercase", padding:"2px 7px", borderRadius:10, background:"rgba(245,158,11,0.14)", color:"#FCD34D", border:"1px solid rgba(245,158,11,0.35)" }}>
            {hasIncomingOutput ? "desde resultado" : "referencia"}
          </span>
      ...
      <div className="node-v2-body">
        ...
        {/* --- MODO OUTPUTNODE CONECTADO: selección interactiva --- */}
        {hasIncomingOutput && (
          <>
            <div className="field-label">Selecciona para referencia de personaje ({selectedFromOutput.size}/{incomingOutputItems.length})</div>
            <div className="ref-slot-row">
              {incomingOutputItems.map((item, i) => {
                const isSel = selectedFromOutput.has(item.url);
                return (
                  <div
                    key={i}
                    onClick={() => toggleOutput(item.url)}
                    className="ref-slot ref-slot-sm"
                    style={{
                      backgroundImage: `url(${item.url})`,
                      backgroundSize: "cover",
                      backgroundPosition: "center",
                      cursor: "pointer",
                      outline: isSel ? "2px solid #F59E0B" : "2px solid transparent",
                  ...
                  >
                    {isSel && (
                      <div style={{ position:"absolute", top:2, right:2, width:14, height:14, borderRadius:"50%", background:"#F59E0B", display:"flex", alignItems:"center", justifyContent:"center", zIndex:2 }}>
            ...
            {d.status === "running" && (
              <div style={{ marginTop:8, padding:"6px 10px", borderRadius:6, background:"rgba(245,158,11,0.08)", border:"1px solid rgba(245,158,11,0.2)", fontSize:10, color:"#FCD34D" }}>
                🎭 Analizando personaje…
              </div>
            )}
            {d.status === "done" && d.characterPrompt && !d.status !== "running" && (
              <div style={{ marginTop:6, padding:"5px 8px", borderRadius:6, background:"rgba(34,197,94,0.08)", border:"1px solid rgba(34,197,94,0.25)", fontSize:10, color:"#86efac" }}>
                ✓ Personaje listo · se inyecta en el flujo
              </div>
            )}
          </>
        )}

        {/* --- MODO MANUAL: subida de archivos --- */}
        {!hasIncomingOutput && (
          <>
        ...
        {/* --- CHARACTER PROMPT (ambos modos) --- */}
        {d.characterPrompt && (
          <>
            <div className="field-label" style={{ marginTop:10 }}>Character Prompt · pasa al Prompt Node</div>
...
// ---------------------------------------------------------------------------
function ImageRefNode({ node, onChange, onMouseDownHeader, onClose, selected, incomingOutputItems }) {
  const accent = "#F97316";
  const d = node.data;
  const images = d.images || [];
  const fileRef = React.useRef(null);

  // --- MODO OUTPUTNODE: selección interactiva ---
  const hasIncoming = incomingOutputItems && incomingOutputItems.length > 0;
  const [selectedOut, setSelectedOut] = React.useState(() => new Set());

  // Limpiar selecciones que ya no existan en el OutputNode
  React.useEffect(() => {
    if (!hasIncoming) return;
    const available = new Set((incomingOutputItems || []).map((it) => it.url));
    setSelectedOut((prev) => new Set([...prev].filter((u) => available.has(u))));
  }, [JSON.stringify((incomingOutputItems || []).map((it) => it.url)), hasIncoming]);

  // Sync selección → images del nodo
  React.useEffect(() => {
    if (!hasIncoming) return;
    const urls = [...selectedOut];
    if (JSON.stringify(urls) !== JSON.stringify(d.images || [])) {
      onChange({ images: urls });
    }
  }, [selectedOut, hasIncoming]);
  ...
  const addImages = async (files) => {
    ...
    for (const file of arr) {
      ...
      try {
        ...
        httpUrls.push(data.url && data.persisted ? data.url : b64);
      } catch (_) { httpUrls.push(b64); }
    }
  ...
  return (
    <div className={"node-v2 imageref-node" + (selected ? " is-selected" : "")}
      ...
      <div className="node-v2-header" onMouseDown={onMouseDownHeader}>
        ...
        <div className="node-v2-right">
          <span className="mono" style={{ fontSize:9, letterSpacing:"0.12em", textTransform:"uppercase", padding:"2px 7px", borderRadius:10, background: hasIncoming ? "rgba(249,115,22,0.22)" : "rgba(249,115,22,0.14)", color:"#FB923C", border:"1px solid rgba(249,115,22,0.3)" }}>
            {hasIncoming ? "desde resultado" : "ref"}
          </span>
          <button className="node-v2-close" onClick={onClose}><Icon.Close style={{ width:11, height:11 }} /></button>
        </div>
      </div>
      ...
      <div className="node-v2-body" style={{ padding:"10px 12px 12px" }}>
        ...
        {/* --- MODO OUTPUTNODE: thumbnails seleccionables --- */}
        {hasIncoming && (
          <>
            <div className="field-label" style={{ marginBottom:6 }}>
              Selecciona las que usarás como referencia ({selectedOut.size}/{incomingOutputItems.length})
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:6, marginBottom:8 }}>
              {incomingOutputItems.map((item, i) => {
                const isSel = selectedOut.has(item.url);
                return (
                  <div
                    key={i}
                    onClick={() => toggleOut(item.url)}
                    style={{
                      position:"relative", aspectRatio:"1", borderRadius:8, overflow:"hidden",
                      background:"rgba(255,255,255,0.04)",
                      outline: isSel ? "2px solid #F97316" : "2px solid transparent",
                  ...
                  >
                    <img src={item.url} alt="" style={{ width:"100%", height:"100%", objectFit:"cover", display:"block" }}
                      onError={(e) => { const el=e.currentTarget; if(!el.dataset.p && window.__proxied){ el.dataset.p="1"; el.src=window.__proxied(item.url); } }} />
                    {isSel && (
                      <div style={{ position:"absolute", top:3, right:3, width:14, height:14, borderRadius:"50%", background:"#F97316", display:"flex", alignItems:"center", justifyContent:"center" }}>
                        <span style={{ color:"#000", fontSize:9, fontWeight:700, lineHeight:1 }}>✓</span>
                      </div>
                    )}
                  </div>
            ...
            {selectedOut.size > 0 && (
              <div className="mono" style={{ fontSize:9.5, opacity:0.55, textAlign:"center", letterSpacing:"0.04em" }}>
                🟢 {selectedOut.size} imagen{selectedOut.size !== 1 ? "es" : ""} seleccionada{selectedOut.size !== 1 ? "s" : ""} · listas para KIE.ai
              </div>
            )}
          </>
        )}

        {/* --- MODO MANUAL: grid + subida --- */}
        {!hasIncoming && (
          <>
            {images.length > 0 && (
...
Object.assign(window, {
  ...
  KID_VIDEO_MODELS,
  Icon,
  // A3: nodos memoizados — arrastrar/editar un nodo no re-renderiza los demás
  // si sus props no cambian (shallow compare). Correctness-safe: peor caso = igual
  // que antes. Las props pesadas (node, callbacks) se comparan por referencia.
  PromptNode: React.memo(PromptNode),
  ImageNode: React.memo(ImageNode),
  VideoNode: React.memo(VideoNode),
  VoiceNode: React.memo(VoiceNode),
  NoteNode: React.memo(NoteNode),
  ReferenceNode: React.memo(ReferenceNode),
  ImageRefNode: React.memo(ImageRefNode),
  OutputNode: React.memo(OutputNode),
  StatusDot,
  ShaderLoader,
  AgentPicker,
});
