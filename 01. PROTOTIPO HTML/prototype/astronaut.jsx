 ...
 *
 * Replica la referencia: robot chibi blanco con traje espacial,
 * visor galaxy púrpura con ojos pill blancos, líneas LED violeta,
...
function AstronautLoop({ active, nodeStatus, briefLength = 0 }) {
  ...
  const ssRef    = React.useRef(null);   // sceneState
  const stRef    = React.useRef({
    energy: 0, targetEnergy: 0,
    lookX: 0,  lookY: 0,
    flash: 0,  shake: 0,
    blinkT: 0, waveT: 0,
    ringW: [0,0,0,0,0],    // per-agent ring weights
    errorR: 0,
  });

  /* ──────────────────────────────────────────────────────
     SETUP Three.js (mount una sola vez)
  ────────────────────────────────────────────────────── */
  React.useEffect(() => {
    ...
    const visorsurface = new THREE.MeshBasicMaterial({
      color: 0x0a0010,
      transparent: true,
      opacity: 0.92,
    });

    /* ── Root group (floats + rotates with mouse) ── */
    ...
    /* Chest badge — Cliender logo mark (simple rounded rect) */
    ...
    /* LED strips on body — violet lines */
...
window.AstronautLoop = AstronautLoop;
