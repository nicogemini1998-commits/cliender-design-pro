...
// ---------------------------------------------------------------------------
function CanvasNode({ node, selected, onSelect, onDrag, onGroupDragStart, onDataChange, onClose, onGenerate, hasIncomingPrompt, incomingPrompt, incomingMedia, incomingRefImages, incomingOutputItems, onOutputAction, activeClient, activeMoodboard, onResizeCorner }) {
  return (
    <DraggableWrap node={node} selected={selected} onSelect={onSelect} onDrag={onDrag} onDragStart={node.type === 'group' ? onGroupDragStart : undefined}>
      {({ onMouseDownHeader }) => {
        ...
        if (node.type === "reference") return <ReferenceNode {...common} incomingOutputItems={incomingOutputItems || null} />;
        if (node.type === "imageref")  return <ImageRefNode  {...common} incomingOutputItems={incomingOutputItems || null} />
...
function ContextMenu({ menu, onPick, onClose }) {
  ...
  const items = onNode ? [
    ...
    { action: "add-imageref", label: "Crear Refs. Imágenes",  hint: "fotos producto / estilo", accent: "#F97316", icon: <span style={{fontSize:17,lineHeight:1}}>🖼</span> },
    { action: "add-group",    label: "Crear grupo",            hint: "agrupar nodos",           accent: "#94A3B8", icon: <span style={{fontSize:14,lineHeight:1}}>⬚</span> },
  ];
...
function App() {
  ...
  useEffect(() => {
    ...
    window.__moodboards.poll((remote) => {
      ...
      for (const rmt of remote) {
        ...
        // Si el id está en tombstones, fue borrado a propósito: ignorar localmente (NO borrar del server).
        if (mbTombstonesRef.current.has(rmt.id)) { continue; }
  ...
  const EDGE_COMPAT = {
    ...
    voice:     ["video"],
    note:      [],
    output:    ["prompt", "image", "video", "reference", "imageref"],
  };
  ...
  // Mapa nodeId (ReferenceNode) → items del OutputNode conectado upstream
  // Bidireccional: funciona si el usuario arrastra desde Output→Reference O Reference→Output
  const incomingOutputItems = useMemo(() => {
    const map = {};
    edges.forEach((e) => {
      const src = nodes.find((n) => n.id === e.source);
      const tgt = nodes.find((n) => n.id === e.target);
      if (!src || !tgt) return;
      // Bidireccional + ambos tipos de nodo referencia: "reference" e "imageref"
      const IS_REF = (t) => t === "reference" || t === "imageref";
      let outputNode = null, refNodeId = null;
      if (src.type === "output" && IS_REF(tgt.type)) { outputNode = src; refNodeId = e.target; }
      else if (IS_REF(src.type) && tgt.type === "output") { outputNode = tgt; refNodeId = e.source; }
      else return;
      const allItems = (outputNode.data?.items || []).filter(
        (it) => it && it.url && typeof it.url === "string" && it.url.length > 4
      );
      // Si el usuario marcó una imagen activa (lastUrl), pasar SOLO esa.
      // Si no marcó ninguna, pasar todas para que elija en el ImageRefNode.
      const lastUrl = outputNode.data?.lastUrl;
      const items = lastUrl
        ? allItems.filter((it) => it.url === lastUrl)
        : allItems;
      if (items.length) {
        if (!map[refNodeId]) map[refNodeId] = [];
        map[refNodeId].push(...items);
      }
    });
    return map;
  }, [edges, nodes]);
  ...
  const _runNodeImpl = useCallback(async (nodeId, inheritedBrief = null, inheritedFrameUrl = null) => {
    ...
    if (node.type === "prompt") {
      ...
      const _listaRe = /\blista\s+de\s+(?:(\d+)\s+)?(?:im[áa]genes?|fotos?|posts?|publicaciones?|piezas?|v[íi]deos?|reels?|prompts?|variantes?|creativos?|opciones?)\b/i;
      const _batchMatch = rawBrief.match(_batchRe);
      const _listaMatch = !_batchMatch ? rawBrief.match(_listaRe) : null;
      const _cantidadNode = Math.max(1, parseInt(node.data.cantidad) || 1);
      const _batchCount = _batchMatch
        ? Math.min(parseInt(_batchMatch[1], 10), 30)
        : _listaMatch
          ? Math.min(parseInt(_listaMatch[1], 10) || 5, 30)
          : Math.min(_cantidadNode, 30);
        ...
        try {
          if (isBatchMode) {
            ...
            if (json.refined_prompt && !json.error) {
              ...
              const _numberedListRe = /^\s*\d+[\.]\)\]\s+.+/m;
              const _parsedList = /^\s*\d+[.)]\s+/m.test(finalBrief)
                ? finalBrief.split(/\n+/).filter(l => /^\s*\d+[.)]\s+/.test(l))
                    .map((l, i) => ({ index: i + 1, prompt: l.replace(/^\s*\d+[.)]\s+/, '').trim() }))
                : null;
              if (_parsedList && _parsedList.length >= 2) {
                const allText = _parsedList.map(p => `${p.index}. ${p.prompt}`).join('\n');
                let _pl = _parsedList.map(p => ({ ...p, status: 'pending' }));
                patchNodeData(nodeId, { status: 'done', agentOutput: allText, prompts: _pl });
                window.__notify?.({ kind: 'success', icon: '✦',
                  title: `${agentObj.name} • ${_parsedList.length} prompts`,
                  body: 'Ejecutando secuencialmente (1 por 1)…' });
                for (let bi = 0; bi < _parsedList.length; bi++) {
                  const item = _parsedList[bi];
                  _pl = _pl.map((p, i) => i === bi ? { ...p, status: 'running' } : p);
                  patchNodeData(nodeId, { _refinedBrief: item.prompt, _batchStatus: `${bi+1}/${_parsedList.length}`, prompts: _pl });
                  for (const e of downstream) await runNode(e.target, item.prompt);
                  _pl = _pl.map((p, i) => i === bi ? { ...p, status: 'done' } : p);
                  patchNodeData(nodeId, { prompts: _pl });
                  if (bi < _parsedList.length - 1) await new Promise(r => setTimeout(r, 300));
                }
                setTimeout(() => patchNodeData(nodeId, { _refinedBrief: undefined, _batchStatus: undefined }), 200);
                setRunningNodes((s) => { const n = new Set(s); n.delete(nodeId); return n; });
                return;
    ...
    async function tryGenerate(genBody, retryCount = 0, taskId = null) {
      ...
      try {
        ...
        const nextTask = (data && data.task_id && String(data.task_id).length > 3) ? data.task_id : taskId;
        ...
        // Sin task_id pero con error de timeout (KIE no asignó task): reintentar generación completa hasta 3 veces.
        if (!nextTask && /timeout|client_timeout/i.test((data && data.error) || "") && retryCount < 3) {
          window.__notify?.({
            kind: "info", icon: "⟳", title: "KIE timeout — reintentando",
            body: `Intento ${retryCount + 1}/3 de nueva generación…`,
          });
          ...
          <div className="canvas-world" style={{ transform }}>
            ...
            {nodes.map((n) => {
              ...
              <CanvasNode
                ...
                incomingOutputItems={incomingOutputItems[n.id] || null}
