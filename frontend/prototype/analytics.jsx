...
const API = window.CDPRO_CONFIG.API_BASE;
...
function PricingTab() {
  ...
  useEffect(() => {
    fetch(`${API}/analytics/pricing`)
      ...
      .then((data) => {
        setRows(data);
        // Inicializar overrides desde datos
        const init = {};
        data.forEach((r) => {
          init[r.model] = {
        ...
        <table className="an-table">
          <thead>
            <tr>
              <th>Modelo</th>
              <th>Prov.</th>
              <th className="an-num">Base ↑/1k</th>
              <th className="an-num">Base ↓/1k</th>
              <th className="an-num">Override ↑/1k</th>
              <th className="an-num">Override ↓/1k</th>
              <th></th>
            </tr>
          </thead>
              ...
              <tr key={r.model}>
                ...
                <td className="an-num">
                  <input
                    ...
                    onChange={(e) => setEditing((s) => ({
                      ...s, [r.model]: { ...s[r.model], override_per_1k_out: e.target.value }
                    }))}
                  />
                </td>
                <td>
                  <button
                    className="an-btn sm accent"
                    disabled={saving[r.model]}
                    onClick={() => save(r.model)}
                  >
        ...
        <div key={a.id} className="an-alert-row">
          ...
          <label className="an-toggle">
            <input
              ...
              onChange={(e) => setEditing((s) => ({
                ...s, [a.id]: { ...s[a.id], enabled: e.target.checked }
              }))}
            />
          ...
          <input
            className="an-input sm"
            type="number"
            step="0.5"
            min="0"
            value={editing[a.id]?.limit_usd ?? ""}
            onChange={(e) => setEditing((s) => ({
              ...s, [a.id]: { ...s[a.id], limit_usd: parseFloat(e.target.value) }
            }))}
          />
          <button
            className="an-btn sm accent"
        ...
        <div className="an-body">
          {tab === "overview" && (
            <>
              ...
              {loading && (
                <div className="an-loading">Cargando datos…</div>
              )}
              {error && (
              ...
              {summary && !loading && (
                <>
                  ...
                  <div className="an-stats-row">
                    <StatCard
                      label="Gasto total"
                      value={fmtShort$(summary.total_cost_usd)}
                    ...
                    <StatCard
                      ...
                      value={fmtNum(summary.total_tokens_in)}
                    />
                    <StatCard
                      label="Tokens salida"
                      value={fmtNum(summary.total_tokens_out)}
                    />
                  </div>

                  {/* Alerta barra de progreso */}
                    ...
                    <div className="an-limit-bar-wrap">
                      ...
                      <div className="an-limit-bar">
                        <div
                          ...
                          style={{
                            width: Math.min(100, (summary.alert.current_usd / summary.alert.limit_usd) * 100) + "%"
                          }}
                        />
                      </div>
                    </div>
