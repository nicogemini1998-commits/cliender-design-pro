...
function Toast({ kind, title, body, onDismiss, icon }) {
  const palette = {
    ...
    warning: { c: "#F59E0B", bg: "rgba(245,158,11,0.10)", border: "rgba(245,158,11,0.35)" },
  ...
  return (
    <div className="dock">
      ...
      <div className="dock-items">
        {items.map((it) => (
          <button key={it.type} className={"dock-item is-" + it.type} onClick={() => onAdd(it.type)} title={it.hint}>
        ...
        <div className="gallery-head">
          ...
          <div className="gallery-filters">
            {[{ k: "all", label: "Todo" }, { k: "image", label: "Imagen" }, { k: "video", label: "Video" }].map((f) => (
              <button key={f.k} className={"gallery-filter " + (filter === f.k ? "is-active" : "")} onClick={() => setFilter(f.k)}>
        ...
        <div className="gallery-body scroll-thin">
          {filtered.length === 0 ? (
            <div className="gallery-empty">
              ...
              <div className="vault-empty-body">Genera tu primera creación desde un nodo Imagen o Video y aparecerá aquí.</div>
            </div>
          ) : (
            <div className="gallery-grid">
              {filtered.map((it) => (
                <button key={it.id} className="gallery-card" onClick={() => onSelect && onSelect(it)}>
                  ...
                  <button className="gallery-remove" onClick={(e) => { e.stopPropagation(); onRemove(it.id); }}>✕</button>
                </button>
              ))}
            </div>
          )}
