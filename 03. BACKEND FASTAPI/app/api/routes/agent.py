...
class BatchRunRequest(BaseModel):
    ...
    outputType: str = "image"               # "image" | "video"
    client: Optional[ClientContext] = None
    reference_images: Optional[list[str]] = None  # URLs upstream para consistencia de personaje
...
def _build_batch_system(req: BatchRunRequest) -> str:
    ...
    batch_instructions = [
        "",
        "=== MODO BATCH — OVERRIDE DE OUTPUT FORMAT ===",
        "IMPORTANTE: Las reglas de OUTPUT FORMAT anteriores NO aplican en este modo.",
        ...
        "- Si hay imágenes de referencia de personaje, cada prompt DEBE mantener la identidad del personaje.",
        "- Formato de respuesta ESTRICTO (sin texto extra, sin explicaciones):",
        "  1. <prompt uno completo en inglés>",
        "  2. <prompt dos completo en inglés>",
        "  ...",
        f"  {count}. <prompt {count} completo en inglés>",
        "- Cada prompt en una sola línea, en inglés, completo y autocontenido.",
        "- Empieza directamente con '1.' sin prólogo ni conclusión.",
    ]
