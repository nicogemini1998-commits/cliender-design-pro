...
def _build_ass_cinematic(segments: list, font_size: int, position: str) -> str:
    """Open Sans Bold, sentence case, outline negro grueso. Estilo cinematografico limpio."""
    fs = max(48, int(font_size * 1.2))
    a = _align_num(position); mv = _mv(position)
    style = (
        f"Style: Default,Open Sans,{fs},"
        "&H00FFFFFF,&H000000FF,&H00000000,&H00000000,"
        f"-1,0,0,0,100,100,0,0,1,4,2,{a},{_MARGIN_LR},{_MARGIN_LR},{mv},1"
    )
...
def _build_ass_editorial(segments: list, font_size: int, position: str) -> str:
    """Liberation Serif itálico dorado + blanco. Dos capas escalonadas. Estilo editorial reel."""
    fs_g = max(56, int(font_size * 1.35))
    fs_w = max(36, int(font_size * 0.85))
    gold  = _bgr("#C9A42A")
    mv    = _mv(position)
    ml    = _MARGIN_LR
    gold_s = (
        f"Style: Gold,Liberation Serif,{fs_g},"
        f"{gold},&H000000FF,&H00000000,&H00000000,"
        f"0,-1,0,0,100,100,3,0,0,0,0,1,{ml},{ml},{mv + 65},1"
    )
    white_s = (
        f"Style: White,Liberation Serif,{fs_w},"
        "&H00FFFFFF,&H000000FF,&H00000000,&H00000000,"
        f"0,-1,0,0,100,100,2,0,0,0,0,1,{ml + 100},{ml},{mv},1"
    )
    hdr = (
        "[Script Info]\nScriptType: v4.00+\n"
        f"PlayResX: {_PLAY_W}\nPlayResY: {_PLAY_H}\n"
    ...
    for seg in segments:
        ...
        g_txt = " ".join(words[:mid])
        w_txt = " ".join(words[mid:]) if len(words) > mid else None
        s, e = seg["start"], seg["end"]
        lines.append(f"Dialogue: 0,{_ts_ass(s)},{_ts_ass(e)},Gold,,0,0,0,,{_esc(g_txt)}\n")
        if w_txt:
            w_s = min(s + max(0.2, (e - s) * 0.35), e - 0.1)
            lines.append(f"Dialogue: 1,{_ts_ass(w_s)},{_ts_ass(e)},White,,0,0,0,,{_esc(w_txt)}\n")
    return "".join(lines)
...
class VideoEditRequest(BaseModel):
    ...
    def _preset_only(cls, v: str) -> str:
        return v if v in ("classic", "cinematic", "editorial", "pop", "box", "karaoke") else "classic"
        ...
        if req.burn_subtitles and src_has_audio:
            ...
            if segments or words:
                ...
                if preset == "pop":
                    ...
                    content = _build_ass_cinematic(segments, st.font_size, st.position)
                    sub_path = os.path.join(tmp, "subs.ass")
                    sub_is_ass = True
                elif preset == "editorial":
                    content = _build_ass_editorial(segments, st.font_size, st.position)
                    sub_path = os.path.join(tmp, "subs.ass")
                    sub_is_ass = True
                elif preset == "box":
