...
import { presentationFor, timingFor } from "./transitions-presentations.jsx";
...
export const LOOKS = {
  ...
  kodak: {
    label: "Kodak Vision3",
    description: "Emulación de Kodak Vision3 500T: sombras cálidas terrosas, medios desaturados, altas luces limpias. El look fílmico del cine indie.",
    filter: "saturate(0.82) contrast(1.09) brightness(0.97)",
    shadows: "#2B1505", shadowsOpacity: 0.28,
    highlights: "#FFF8E8", highlightsOpacity: 0.07,
    liftBlacks: 0.04,
    grainBoost: 0.60,
    vignetteBoost: 0.75,
  },
  teal_orange: {
    label: "Teal & Orange",
    description: "Dúo complementario Hollywood: sombras teal, pieles y luces naranja. Blockbuster, alto contraste.",
    filter: "saturate(1.18) contrast(1.14)",
    shadows: "#003344", shadowsOpacity: 0.30,
    highlights: "#FF7A22", highlightsOpacity: 0.13,
    liftBlacks: 0,
    grainBoost: 0.25,
    vignetteBoost: 0.55,
  },
  muted: {
    label: "Muted / Fade",
    description: "Estética desaturada tipo editorial de moda. Tonos apagados, sombras levantadas. Elegancia melancólica.",
    filter: "saturate(0.55) contrast(0.9) brightness(1.05)",
    shadows: "#8A7B6E", shadowsOpacity: 0.22,
    highlights: "#F0EDE8", highlightsOpacity: 0.10,
    liftBlacks: 0.08,
    grainBoost: 0.45,
    vignetteBoost: 0.40,
  },
  velvia: {
    label: "Velvia",
    description: "Fuji Velvia: colores saturados, verdes y rojos muy vivos, contraste fuerte. Naturaleza, outdoor, reels de energía.",
    filter: "saturate(1.45) contrast(1.18) brightness(0.98)",
    shadows: "#001520", shadowsOpacity: 0.20,
    highlights: "#F0FF88", highlightsOpacity: 0.06,
    liftBlacks: 0,
    grainBoost: 0.15,
    vignetteBoost: 0.50,
  },
  bleach: {
    label: "Bleach Bypass",
    description: "Proceso de laboratorio: alto contraste, desaturación intensa. Thrillers, drama urbano. Look 'Se7en'.",
    filter: "saturate(0.38) contrast(1.45) brightness(0.92)",
    shadows: "#0A0A08", shadowsOpacity: 0.25,
    highlights: "#F5F5F0", highlightsOpacity: 0.05,
    liftBlacks: 0,
    grainBoost: 0.65,
    vignetteBoost: 0.85,
  },
  analog: {
    label: "Analog",
    description: "Película de los 90: dominante naranja-amarillenta, grano visible. Nostálgico y cálido.",
    filter: "saturate(0.88) contrast(1.05) brightness(0.99) sepia(0.12)",
    shadows: "#0F1A0A", shadowsOpacity: 0.20,
    highlights: "#FFE8C0", highlightsOpacity: 0.10,
    liftBlacks: 0.02,
    grainBoost: 0.70,
    vignetteBoost: 0.60,
  },
  dusk: {
    label: "Dusk / Dreamy",
    description: "Tonos crepusculares: púrpuras en sombras, luces doradas. Atmósfera onírica para belleza o lifestyle.",
    filter: "saturate(1.08) contrast(1.06) brightness(1.02)",
    shadows: "#2A0A3A", shadowsOpacity: 0.32,
    highlights: "#FFD080", highlightsOpacity: 0.11,
    liftBlacks: 0.03,
    grainBoost: 0.30,
    vignetteBoost: 0.65,
  },
  instagram: {
    label: "Instagram Warm",
    description: "El filtro universal de IG: ligeramente caliente, +brillo, vignette suave. Atractivo inmediato para RRSS.",
    filter: "saturate(1.12) contrast(1.08) brightness(1.08) sepia(0.08)",
    shadows: "#1A0A00", shadowsOpacity: 0.18,
    highlights: "#FFF0D0", highlightsOpacity: 0.13,
    liftBlacks: 0.02,
    grainBoost: 0.10,
    vignetteBoost: 0.35,
  },
...
export const TRANSITIONS = {
  ...
  dipwhite: {
    label: "Dip a blanco",
    description: "Flash a blanco entre clips: impacto, recuerdo o golpe emocional. Usar con criterio.",
    frames: (fps) => Math.round(fps * 0.5),
  },
  blurwipe: {
    label: "Blur wipe",
  ...
  flipx: {
    label: "Flip 3D horizontal",
    description: "El clip gira en 3D sobre su eje vertical y revela el siguiente en el reverso. Espectacular y premium.",
    frames: (fps) => Math.round(fps * 0.55),
  },
  flipy: {
    label: "Flip 3D vertical",
    description: "Giro 3D sobre el eje horizontal: el clip cae hacia delante mostrando el siguiente. Dinamico y moderno.",
    frames: (fps) => Math.round(fps * 0.55),
  },
  clockwipe: {
    label: "Clock wipe",
    description: "Barrido radial tipo manecilla de reloj que revela el clip nuevo girando desde el centro. Editorial y llamativo.",
    frames: (fps) => Math.round(fps * 0.6),
  },
  cut: {
    label: "Corte seco",
...
function computeTimeline(scenes, fps, branding) {
  ...
  scenes.forEach((scene, i) => {
    ...
    const td = noPrev ? 0 : Math.min(transitionFrames(scene, fps), Math.floor(prevDur * 0.9), Math.floor(dur * 0.9));
...
// ──────────────────────────────────────────────────────────────────────────
// SUBTÍTULOS — sistema dinámico: position + captionStyle por escena.
//   position:     "bottom" | "top" | "center" | "left" | "right"
//   captionStyle: "pill" | "outline" | "stamp" | "gradient" | "minimal" | "bold" | "kinetic"
// ──────────────────────────────────────────────────────────────────────────

function posStyle(position, letterbox) {
  const padV = letterbox ? 210 : 170;
  const padT = letterbox ? 180 : 120;
  switch (position) {
    case "top":
      return { justifyContent: "flex-start", alignItems: "center", padding: `${padT}px 54px 0` };
    case "center":
      return { justifyContent: "center", alignItems: "center", padding: "0 54px" };
    case "left":
      return { justifyContent: "flex-end", alignItems: "flex-start", padding: `0 54px ${padV}px 54px` };
    case "right":
      return { justifyContent: "flex-end", alignItems: "flex-end", padding: `0 54px ${padV}px 54px` };
    default:
      return { justifyContent: "flex-end", alignItems: "center", padding: `0 54px ${padV}px` };
  }
}

function CaptionText({ text, accent, variant }) {
  if (variant === "outline") {
    return (
      <span style={{
        ...
        textShadow: "0 3px 24px rgba(0,0,0,0.9), 0 0 8px rgba(0,0,0,0.8)",
        maxWidth: "100%",
        textAlign: "center",
      }}>{text}</span>
    );
  ...
  if (variant === "bold") {
    return (
      <span style={{
        ...
        letterSpacing: "-0.02em",
        textTransform: "uppercase",
        textShadow: "0 4px 0 rgba(0,0,0,0.6), 0 8px 32px rgba(0,0,0,0.8)",
...
function KineticCaption({ text, accent, frame, inStart, position, letterbox }) {
  const chars = text.split("");
  const container = posStyle(position, letterbox);
  return (
    <AbsoluteFill style={{ ...container, zIndex: 45 }}>
      <div style={{ textAlign: "center", maxWidth: "100%" }}>
        <div style={{
          ...
          {chars.map((ch, i) => {
            const d = inStart + i * 1.8;
            const op = interpolate(frame, [d, d + 7], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
            const dy = interpolate(frame, [d, d + 9], [18, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
            return (
              <span key={i} style={{
                display: "inline-block",
                opacity: op,
...
function CaptionPill({ text, accent, opacity, rise, letterbox, position = "bottom", captionStyle = "pill" }) {
  const container = posStyle(position, letterbox);
  return (
    <AbsoluteFill style={{ ...container, zIndex: 45 }}>
      <div style={{ opacity, transform: `translateY(${rise}px)`, textAlign: "center", maxWidth: "100%" }}>
        <CaptionText text={text} accent={accent} variant={captionStyle} />
      </div>
    </AbsoluteFill>
  );
}

function CinematicCaption({ text, accent, frame, fps, letterbox, sceneDur, position = "bottom", captionStyle = "pill" }) {
  if (!text) return null;
  const inStart = Math.min(Math.round(fps * 0.3), Math.max(0, Math.round((sceneDur || fps * 3) * 0.15)));
  if (captionStyle === "kinetic") {
    return <KineticCaption text={text} accent={accent} frame={frame} inStart={inStart} position={position} letterbox={letterbox} />;
  }
  ...
  return <CaptionPill text={text} accent={accent} opacity={opacity} rise={rise} letterbox={letterbox} position={position} captionStyle={captionStyle} />;
}

function TimedCaptions({ segments, accent, frame, fps, letterbox, position = "bottom", captionStyle = "pill" }) {
  const t = frame / fps;
  ...
  return <CaptionPill text={seg.text} accent={accent} opacity={opacity} rise={rise} letterbox={letterbox} position={position} captionStyle={captionStyle} />;
}
...
function TikTokCaptions({ words, accent, frame, fps, letterbox, variant, position = "bottom" }) {
  ...
  const padV = letterbox ? 250 : 210;
  const container = position === "bottom"
    ? { justifyContent: "flex-end", alignItems: "center", padding: `0 40px ${padV}px` }
    : posStyle(position, letterbox);

  return (
    <AbsoluteFill style={{ ...container, zIndex: 46 }}>
    ...
    <AbsoluteFill style={{ background: "#000" }}>
      <AbsoluteFill style={{ background: "#000", overflow: "hidden", ...wrapper }}>
        ...
        <AbsoluteFill style={{ filter: look.filter === "none" ? undefined : look.filter, transform: style.autopunch && isVideo ? `scale(${punchScale(scene.beats, frame, fps)})` : undefined }}>
          {isVideo ? (
            <OffthreadVideo src={scene.url} muted={scene.muted !== false} playbackRate={Number(scene.speed) > 0 ? Number(scene.speed) : 1} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
          ) : (
        ...
        {Array.isArray(scene.words) && scene.words.length && TIKTOK_VARIANTS.has(style.captions) ? (
          <TikTokCaptions words={scene.words} accent={accent} frame={frame} fps={fps} letterbox={style.letterbox} variant={style.captions} position={scene.captionPosition || "bottom"} />
        ) : Array.isArray(scene.segments) && scene.segments.length ? (
          <TimedCaptions segments={scene.segments} accent={accent} frame={frame} fps={fps} letterbox={style.letterbox} position={scene.captionPosition || "bottom"} captionStyle={scene.captionStyle || "pill"} />
        ) : (
          <CinematicCaption text={scene.caption} accent={accent} frame={frame} fps={fps} letterbox={style.letterbox} sceneDur={durationInFrames} position={scene.captionPosition || "bottom"} captionStyle={scene.captionStyle || "pill"} />
        )}
...
const SFX_FOR = {
  ...
  flipx: [["whoosh", 0.5]],
  flipy: [["whoosh", 0.5]],
  clockwipe: [["whoosh", 0.42]],
  dipwhite: [["boom", 0.4], ["whoosh", 0.3]],
};
...
export function Stitch({ scenes = [], brand = {}, fps = 30, style: styleProp = {} }) {
  ...
  const style = {
    ...
    sfxOn: sfx,
    dipColor: typeof styleProp.dipColor === "string" ? styleProp.dipColor : undefined,
  };
  const { items, total } = computeTimeline(scenes, fps, branding);
  const { width, height } = useVideoConfig();

  // Hijos del TransitionSeries: una Sequence por clip y una Transition de DOS
  // CARAS entre clips. td<=0 ⇒ corte seco (sin Transition entre las Sequences).
  ...
  items.forEach((it, idx) => {
    ...
    let tid;
    if (it.type === "intro") {
      content = <BrandCard brand={brand} kind="intro" />;
      dur = it.dur;
    } else if (it.type === "outro") {
      content = <BrandCard brand={brand} kind="outro" />;
      dur = it.dur;
      tid = "dissolve";
    } else {
      content = (
        <SceneClip scene={it.scene} index={it.index} accent={accent} transition={transitionId(it.scene)} td={0} look={(it.scene.look && LOOKS[it.scene.look]) ? LOOKS[it.scene.look] : look} style={style} />
      );
      dur = it.dur;
      tid = transitionId(it.scene);
    }
    // Transition ANTES de este clip (salvo el primero) cuando hay solape real.
    if (!isFirst && it.td > 0 && tid) {
      seriesChildren.push(
        <TransitionSeries.Transition
          key={`t${idx}`}
          presentation={presentationFor(tid, { width, height, dipColor: style.dipColor })}
          timing={timingFor(tid, it.td)}
        />
      );
    }
    seriesChildren.push(
      <TransitionSeries.Sequence key={`q${idx}`} durationInFrames={dur}>
        {content}
  ...
  return (
    <AbsoluteFill style={{ background: "#000" }}>
      ...
      {/* Capas globales: sonido, grano, letterbox y fundidos de pelicula */}
